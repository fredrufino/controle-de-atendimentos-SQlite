...
def vizualizar_chamados_completo(conn): #Chamados concluidos
    try:
        ...
        resultado = c.fetchall()
        print(f"{co.BOLD}{co.OKGREEN}")
        print("{:<2} {:<15} {:<5} {:<5} {:<10} {:<8} {:<15} {:<15} {:<10} {:<10} {:<25} {:<5} {:<2} {:<5}".format("ID", "Descrição", "Data Con.", "Data Soli.", "Ações", "Status", "Nome Tel.", "Tipo local", "Cidade", "Rua", "Número", "Complemento", "Avaliação", "Data Avaliação"))
        print(f"{co.ENDC} ")
        for item in range(len(resultado)):
            print("{:<2} {:<15} {:<5} {:<5} {:<10} {:<8} {:<15} {:<15} {:<10} {:<10} {:<25} {:<5} {:<2} {:<5}".format(resultado[item][0], resultado[item][1], resultado[item][2], resultado[item][3], resultado[item][4], resultado[item][5], resultado[item][6], resultado[item][7], resultado[item][8], resultado[item][9], resultado[item][10], resultado[item][11], resultado[item][12], resultado[item][13]))
        conn.commit()
    except Error as e:
        print(f"ERRO {e} na hora de mostar chamados concluidos CHAMADO")
        time.sleep(7)
...
def inserir(conn, tabela): #Opcao inserir no Sub Menu
    if tabela == "Usuario":
        ...
        try:
            ...
            c.execute(f"SELECT NOME FROM USUARIO WHERE SIGLA_USUARIO = 'TEL' AND ID_USUARIO = {id_tel};") #Pega o Nome do Telefonista
